...
const AddJob = () => {
  const {user:{companyName}} = useSelector(state => state.firebase)
  const { handleSubmit, register, control } = useForm({
    defaultValues:{
      companyName
    }
  });
  const [postJob, {isLoading, isError, isSuccess, error}] = usePostJobMutation()
  ...
  useEffect(()=>{
    if(isSuccess){
      toast.success("Job Post Success", {id:"job-post"})
    }
    if(isError){
      toast.error(error, {id:"job-post"})
    }
    if(isLoading){
      toast.loading("Please Wait...", {id:"job-post"})
    }
  },[isSuccess, isError])
