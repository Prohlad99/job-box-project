...
const CandidateRegistration = () => {
  ...
  const [postUser, {isSuccess, isError, error}] = useRegisterMutation()
  ...
  useEffect(()=>{
    if(isSuccess){
      toast.success("Registration Success.", {id:"candidate"})
    }

    if(isError){
      toast.error(error, {id:"candidate"})
    }
  },[])
  return (
    <div className='pt-14'>
      ...
      <div className='flex justify-center items-center overflow-auto p-10'>
        ...
        >
          <h1 className='w-full text-2xl text-primary mb-5 text-center'>Create An Account As Candidate</h1>
          ...
          <div className='flex flex-col w-full max-w-xs'>
            ...
            <select {...register("country")} id='country'>
              {countries
                ...
                .map(({ name }) => (
                  <option key={Math.random()*2342} value={name.common}>{name.common}</option>
                ))}
            </select>
          </div>
          <div className='flex flex-col w-full max-w-xs'>
