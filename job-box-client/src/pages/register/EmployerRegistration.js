...
const EmployerRegistration = () => {
  ...
  const {user:{email}} = useSelector(state => state.firebase)
  ...
  const [postUser, {isLoading, isError, isSuccess, error}] = useRegisterMutation( {refetchOnMountOrArgChange:true})

  useEffect(()=>{
    if(isLoading){
      toast.loading("Please Wait...", {id :"registration"})
    }
    if(isSuccess){
      toast.success("Registration Success.", {id :"registration"})
    }
    if(isError){
  ...
  return (
    <div className='pt-14'>
      ...
      <div className='flex justify-center items-center overflow-auto p-10'>
        ...
        >
          <h1 className='w-full text-2xl text-primary mb-5 text-center'>Create An Account As Employer</h1>
          ...
          <div className='flex flex-col w-full max-w-xs'>
            ...
            <select {...register("employeeRange")} id='employeeRange'>
              {employeeRange
                ...
                .map((category) => (
                  <option key={Math.random()+53432} value={category}>{category}</option>
                ))}
            </select>
          </div>
          ...
          <div className='flex flex-col w-full max-w-xs'>
            ...
            <select {...register("companyCategory")} id='companyCategory'>
              {businessCategory
                .sort((a, b) => a.localeCompare(b))
                .map((category) => (
                  <option key={Math.random()*2432} value={category}>{category}</option>
                ))}
            </select>
          </div>
          <div className='flex flex-col w-full max-w-xs'>
